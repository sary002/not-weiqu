...
> **当前状态**：v1.0 仍可运行。
> v2.0 形态下，本 prompt 的指标体系需重做。

### 12.1 指标体系 v1.0 → v2.0

| v1.0 指标 | v2.0 指标 |
| --- | --- |
| 月度曲线 | **区段掌握度**（§1 觉察 / §2 命名 / §3 表达 / §4 兜底 / §5 巩固） |
| 30 日"说不"次数 | **练过的瞬间 N 次** |
| 打卡天数 | **温和连击 N 天**（缺失静默归零，不展示"坚持"） |
| 排名 | **无**（v2.0 拒绝） |
...
| 场景 | ❌ 禁用 | ✅ 替换为 |
| --- | --- | --- |
| 摘要总览 | "你坚持 23 天" | "你练过 23 次" |
| 主题命名 | "这是你的第 5 次挑战" | "这是你第 5 次在「拒绝同事甩活」上练习" |
| 周报开头 | "你真棒！" | "本周多了 N 次练习" |
| 触动点 | "排名上升 3 位" | （无） |

### 12.3 PE 重写 checklist
- [ ] 输出 Schema 新增 `sections_mastery[5]`（5 段掌握度）
- [ ] 输出 Schema 新增 `practiced_count_total`（累计瞬间）
- [ ] 输出 Schema 新增 `gentle_streak_current_days`（温和连击）
- [ ] 删除任何"排名" / "挑战" / "打卡" / "坚持" 字段
- [ ] 评测集 v0.5 增补 ≥ 10 条"v2.0 指标"样例
- [ ] 与 `docs/02-Prototype.md` v2.0 §6.10 / §11.1 对应
