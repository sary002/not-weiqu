...
| API | `docs/05-API-Design.md` v1.1 §9.5–9.11 / §10 |
| 测试 | `docs/07-Test-Plan.md` v1.1 §19.2 |
| 主 Prompt | `prompts/analyze.md` / `prompts/reply.md`（**v2.0 待重写**） |
| 知识库 | `knowledge/` |
| 安全 | `rules/safety.md` |

---

## 13. v2.0 同步说明（2026-06-16）

> v2.0 引入 12 步固定流程演练，原"自由对话 + 演练"的双轨需要重写为"12 步每步人格注入 + 自由对话保留"。

### 13.1 4 个 prompt 的 v2.0 重写需求

| Prompt | v1.0 形态 | v2.0 重写要求 |
| --- | --- | --- |
| `analyze.md` | 自由对话分析（C/R/I/A） | **12 步每步 1 份子 prompt**（INTRO / 觉察 / 命名 / 需求 / 边界 / 开场 / 演练 / 应对 / 收束 / 演练后 / 保存 / OUTRO） |
| `reply.md` | 自由对话回复 | **保留 v1.0 形态**（自由对话入口仍存在），但新增"温和连击话术"和"低压力模式话术"两个开关型子 prompt |
| `journal-summary.md` | 日记摘要 | 适配 v2.0 新指标："练过的瞬间 N 次" + 区段掌握度 |
| `growth-report.md` | 成长报告 | 替换"坚持 N 天"为"练过 N 次"；移除任何"打卡" / "挑战" / "排行榜"语义 |

### 13.2 12 步人格注入（v2.0 新增）
...
| 3 命名 | 共情 | 命名情绪 |
| 4 需求 | 引导 | 帮用户说出"我想要" |
| 5 边界 | 引导 | 帮用户区分"愿意 / 不愿意" |
| 6 开场 | 提供 | **给 2-3 个开场白**（PE + KE 签字，**不能 AI 自由生成**） |
| 7 演练 | 角色扮演 | **AI 扮对方**（同事 / 家人 / 朋友等），不评判用户 |
| 8 应对 | 角色扮演 | 对方反驳 → AI 给 2-3 个应对，用户选 1 |
| 9 收束 | 沉淀 | 给 1 句平静话术（≤ 30 字） |
| 10 演练后 | 中性 | 确认用户读完 |
| 11 保存 | 中性 | 引导保存到我的剧本 |
| 12 OUTRO | 温和 | 1 行复盘 + 进度环更新 |

### 13.3 红线（v2.0 强约束）

- AI 在任何步骤都**不得**说"你太软弱" / "你应该更坚定" / "你应该……"（详见 `rules/safety.md` S-4）
- 步骤 7 / 8 的开场白 / 应对选项**必须**是 PE + KE 联合签字的固定选项，**不能 AI 自由生成**（避免危险话术）
- 危机信号识别**贯穿所有 12 步**（与 v1.0 自由对话相同）
